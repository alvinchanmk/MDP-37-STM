...
//	  HAL_UART_Transmit(dev->uart, uartbuf, strlen((char*)uartbuf), HAL_MAX_DELAY);
      ...
      ret = IMU_WriteOneByte(dev, REG_ADD_GYRO_SMPLRT_DIV, 0x2C);  // 125hz 0x67 pg 59  Gyro sample rate divider Output data rate = 1.125K/7 = 140 Hz
	  if ( ret != HAL_OK )	  {
      ...
      ret = IMU_WriteOneByte(dev, REG_ADD_ACCEL_SMPLRT_DIV_2,  0x2C); // 0x67 og 0x07//  pg 63 Acce sample rate divider: ODR = 1.125KHz/7 = 161
...
HAL_StatusTypeDef IMU_AccelRead(ICM20948 *dev)
{
	...
	dev->acc[2] = 0.00006103515625f * accRawSigned[2]; //9.81f *
...
{   // return the change in value instead of current value
	...
	dev->gyro[0] = 0.0076335878f * gyroRawSigned[0]; // og val 0.0152671755725191f
	dev->gyro[1] = 0.0076335878f * gyroRawSigned[1]; // og val 0.0152671755725191f
	dev->gyro[2] = 0.0076335878f * gyroRawSigned[2]; // og val 0.0152671755725191f
